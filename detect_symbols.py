...
# from notation2tables import notation2tables
...
def parse_args():
  ...
  parser = argparse.ArgumentParser(
        description='Produces predictions for images in a folder '
        'and saves results in pkl format.')
  ...
  while num_images > 0 or webcam_num > -1:
      ...
      for j in xrange(1, len(pascal_classes)):
          ...
          if inds.numel() > 0:
            ...
            # print("Detections:")
            cls_dets_cpu = cls_dets.cpu()
            # print(cls_dets_cpu.numpy())
            # print(cls_dets_cpu.numpy().shape)

            for di in range(cls_dets_cpu.numpy().shape[0]):
                det_nparray = cls_dets_cpu.numpy()[di, :]
                if det_nparray[4] > args.threshold:
                    det_tuple = (
                            j, *list(det_nparray)
                    )
                    detections.append(det_tuple)
      ...
      # Save pkl with predictions
      pkl_path = os.path.join(args.image_dir, imglist[num_images][:-4] + ".pkl")
