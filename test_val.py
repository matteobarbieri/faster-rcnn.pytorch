...
class sampler(Sampler):
    ...
    def __iter__(self):
        ...
        rand_num = (
            torch.randperm(self.num_per_batch).view(-1, 1) * self.batch_size)
        ...
        if self.leftover_flag:
            self.rand_num_view = (
                torch.cat((self.rand_num_view, self.leftover), 0))
...
if __name__ == '__main__':  # noqa
    ...
    imdb_val, roidb_val, ratio_list_val, ratio_index_val = combined_roidb(
        args.imdbval_name)
    validation_size = len(roidb_val)
    print('{:d} roidb entries (validation)'.format(validation_size))

    # Use validation data for training
    # imdb = imdb_val
    # roidb = roidb_val
    # ratio_list = ratio_list_val
    # ratio_index = ratio_index_val
    ...
    dataset_val = roibatchLoader(
        roidb_val, ratio_list_val, ratio_index_val, args.batch_size,
        imdb_val.num_classes, training=True)
        # imdb_val.num_classes, training=False)
    ...
    if args.net == 'vgg16':
        fasterRCNN = vgg16(
            imdb.classes, pretrained=True,
            class_agnostic=args.class_agnostic)
    elif args.net == 'res101':
        fasterRCNN = resnet(
            imdb.classes, 101, pretrained=True,
            class_agnostic=args.class_agnostic)
    elif args.net == 'res50':
        fasterRCNN = resnet(
            imdb.classes, 50, pretrained=True,
            class_agnostic=args.class_agnostic)
    elif args.net == 'res152':
        fasterRCNN = resnet(
            imdb.classes, 152, pretrained=True,
            class_agnostic=args.class_agnostic)
    else:
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            # Since it is sometimes reset by validation
            fasterRCNN.train()
            ...
            if step % args.disp_interval == 0:
                ...
                # Log losses on tensorboard (training)
                if args.use_tfboard:
                    info = {
                        'loss': loss_temp,
                ...
                data_val = next(data_val_iter)
                # im_data.data.resize_(data_val[0].size()).copy_(data_val[0])
                # im_info.data.resize_(data_val[1].size()).copy_(data_val[1])
                # gt_boxes.data.resize_(data_val[2].size()).copy_(data_val[2])
                # num_boxes.data.resize_(data_val[3].size()).copy_(data_val[3])
                ...
                # TODO apparently calling eval() doesn't work
                # fasterRCNN.eval()
                with torch.no_grad():
                    rois, cls_prob, bbox_pred, \
                    rpn_loss_cls, rpn_loss_box, \
                ...
                # Log losses on tensorboard (validation)
                if args.use_tfboard:
                    info = {
                        'loss_val': loss_temp,
                        'loss_rpn_cls_val': loss_rpn_cls,
                        'loss_rpn_box_val': loss_rpn_box,
                        'loss_rcnn_cls_val': loss_rcnn_cls,
                        'loss_rcnn_box_val': loss_rcnn_box
                    }
                    logger.add_scalars(
                        "logs_s_{}/losses".format(args.session),
                        info, (epoch - 1) * iters_per_epoch + step)
